...
- The service worker that caches the Lista de compras for offline reading (ticket #17) can't be reached by a Session ending, because the device that holds the cached copy never talks to the server to learn it's gone. A device that already has one can keep reading it offline until it's back online (which clears the cache the moment it discovers it's signed out) or the copy passes its one-day cache limit, whichever comes first.
